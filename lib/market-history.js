...
function historyRequestStart(record, oldestRequired) {
  const fetchedFrom = DATE_PATTERN.test(record?.fetchedFrom || '')
    ? record.fetchedFrom
    : null;
  if (!fetchedFrom || fetchedFrom > oldestRequired) return oldestRequired;

  const latestStored = Object.keys(record?.prices || {})
    .filter(date => DATE_PATTERN.test(date))
    .sort()
    .at(-1);
  return latestStored ? addUtcDays(latestStored, -14) : oldestRequired;
}
